...
const copy={
kr:{
 nav:['회사소개','성능','기술','제품','적용분야','지속가능성','연혁','시공사례','파트너십','FAQ','문의'],
 ...
 problem:['산업 현장의 문제','태양빛은 흡수되면','지붕을 가열합니다.','산업 지붕은 태양에너지를 흡수해 건물 내부로 열을 전달합니다.'],
 problemBlocks:[['열이 들어오는 방식','흡수된 열은 전도, 복사와 대류를 통해 지붕 아래 공간으로 이동합니다.'],['현장에 미치는 영향','실내 온도와 냉방 수요가 높아져 작업환경과 에너지 부담에 영향을 줄 수 있습니다.']],
...
en:{
 nav:['About','Performance','Technology','Products','Applications','Sustainability','Timeline','Projects','Partners','FAQ','Contact'],
 ...
 problem:['The challenge','Sunlight heats','industrial roofs.','Industrial roofs absorb solar energy and transfer heat into the building below.'],
 problemBlocks:[['How heat enters','Heat moves through the roof by conduction, radiation and convection.'],['Why it matters','This heat can raise indoor temperatures, affect working conditions and increase cooling demand.']],
...
const ids=['about','performance','technology','product','applications','esg','timeline','projects','partners','faq','contact'];
...
<section id="about" className="panel light"><div className="inner"><Head a={c.about}/><AnswerBlocks items={c.aboutBlocks}/><div className="stats"><div><b>10<sup>°C</sup></b><span>강진군 축사 현장 온도 저감</span></div><div><b>5<sup>단계</sup></b><span>표준 시공 공정</span></div><div><b>AI</b><span>시공 전후 성능관리</span></div></div><small className="note">* 2026.06.07 전남 강진군 축사 현장 실증: 지붕 표면 36°C → 26°C.</small></div></section>
<section id="performance" className="panel navy perf"><div className="inner"><div className="split"><div><Head a={c.problem}/><AnswerBlocks items={c.problemBlocks}/><ol>{c.flow.map((x,i)=><li key={x}><span>0{i+1}</span>{x}</li>)}</ol></div><div className="thermo"><Sun/><div><b>36°C</b><small>{c.chart[1]}</small></div><ArrowDown/><div className="cool"><b>26°C</b><small>{c.chart[2]}</small></div></div></div><div className="proof"><div className="metrics">{c.perfMetrics.map(m=><div key={m[1]}><b>{m[0]}</b><span>{m[1]}</span></div>)}</div><div className="chart"><small>{c.chart[0]}</small><div className="bars"><div className="bar"><em>365,000</em><i style={{height:'210px'}}></i><span>{c.chart[1]}</span></div><div className="bar"><em>264,900</em><i className="lo" style={{height:'152px'}}></i><span>{c.chart[2]}</span></div><b className="save">{c.chart[3]}</b></div></div></div><small className="note">{c.disc}</small></div></section>
